...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
    ...
    override open func execute(_ apiResponseQueue: DispatchQueue = RevoltAPIClient.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, Error>) -> Void) {
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.RevoltAPIClient.\(UUID().uuidString)")
                }
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in RevoltAPIClient.customHeaders {
            httpHeaders[key] = value
        }
        return httpHeaders
    }
    ...
    override fileprivate func processRequestResponse(urlRequest: URLRequest, data: Data?, response: URLResponse?, error: Error?, completion: @escaping (_ result: Swift.Result<Response<T>, Error>) -> Void) {
        ...
        default:
            ...
            case let .failure(error):
                print(error)
                fatalError("Could not decode the response body correctly!")
                // Ideally, do not fatalError and return an error
                // However, this silent error is frustrating and does not make sense for
                // a project that is still heavily a WiP
